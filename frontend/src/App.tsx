...
function MobileHeader() {
  return (
    <header className="md:hidden sticky top-0 z-50 bg-white border-b border-border px-4 py-3 flex items-center justify-between shadow-sm">
      <img src="/oc_logo.png" alt="OC Logo" className="h-8 w-8 object-contain" />
      <nav className="flex items-center gap-5 text-sm font-medium">
        <a href="/home" className="text-text-secondary hover:text-text-primary no-underline transition-colors">Home</a>
        <a href="/profile/1" className="text-text-secondary hover:text-text-primary no-underline transition-colors">Profile</a>
        <button
          onClick={() => logout()}
          className="text-text-secondary hover:text-text-primary bg-transparent border-none cursor-pointer text-sm font-medium p-0 transition-colors"
        >
          Logout
        </button>
      </nav>
